class intCodeComputer:
    def __init__(self, programCode, id=0, relBase=0, indx=0, emptySpace=10000, inptArr=[]):
        self.relativeBase = 0
        self.memory = []
...
#reading data
data = open("Day17_Data.txt").read()
dataSeperated = data.split(",")
for indx in range(len(dataSeperated)):
    dataSeperated[indx] = int(dataSeperated[indx])


#feeding the data to intcode comp
intCopm0 = intCodeComputer(dataSeperated,0)
intCopm0.compute()



#creating a map from intcodeComp output
area=[]
area.append([])
for element in intCopm0.outputArray:
    if(element ==10):
        area.append([])
    else:
        area[len(area)-1].append(chr(element))

#i dont know why, but we have two newline characters in our map
...
for y in range(len(area)):
    for x in range(len(area[y])):
        if(area[y][x] =='#' and checkFourSides(area,x,y)):
            total += x*y
